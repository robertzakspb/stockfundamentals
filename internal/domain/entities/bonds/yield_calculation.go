...
func (b Bond) CalcYieldToMaturity(coupons []Coupon, marketPrice float64) (float64, error) {
	yield, err := calculateYTM(b, coupons, marketPrice, time.Now(), b.MaturityDate)
	if err != nil {
		return -1, err
	}
...
func (b Bond) CalcYieldToCallOption(coupons []Coupon, marketPrice float64) (float64, error) {
	...
	yield, err := calculateYTM(b, coupons, marketPrice, time.Now(), b.CallOptionExerciseDate)
	if err != nil {
		return -1, err
	}
...
func calculateYTM(b Bond, coupons []Coupon, marketPricePercentage float64, acquisitionDate, redemptionDate time.Time) (float64, error) {
	...
	annualCoupon := float64(b.CouponCountPerYear) * b.Coupons[0].PerBondAmount
	marketPriceInCurrency := marketPricePercentage * b.NominalValue / 100
	holdingPeriodInYears := redemptionDate.Sub(acquisitionDate).Hours() / 24 / 365
	numberOfCompoundingPeriods := holdingPeriodInYears * float64(b.CouponCountPerYear)

	ytm := (annualCoupon + (b.NominalValue-marketPriceInCurrency)/numberOfCompoundingPeriods) / ((b.NominalValue + marketPriceInCurrency) / 2)
	return ytm, nil
}
