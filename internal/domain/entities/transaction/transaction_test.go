...
func Test_NewExec_Valid(t *testing.T) {
	...
	exec, err := New(accountId, figi, timestamp, float64(quantity), float64(price), description, side, transactionType, currency)

	test.AssertNoError(t, err)
	test.AssertEqual(t, accountId, exec.AccountId)
	test.AssertEqual(t, figi, exec.Figi)
	test.AssertEqual(t, timestamp, exec.Timestamp)
	test.AssertEqual(t, quantity, exec.Quantity)
	test.AssertEqual(t, price, exec.PricePerUnit)
...
func Test_NewExec_InvalidQuantity(t *testing.T) {
	...
	_, err := New(accountId, figi, timestamp, float64(quantity), float64(price), description, side, transactionType, currency)
...
func Test_TransactionType_Deposit(t *testing.T) {
	...
	transaction, err := New(accountId, figi, timestamp, float64(quantity), float64(price), description, side, transactionType, currency)
