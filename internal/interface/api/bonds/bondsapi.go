...
func StartBondAndCouponImportJob(c *gin.Context) {
	...
	c.JSON(http.StatusOK, shared.StringResponse{Message: "The bond import job has been successfully started"})
}

func StartBondAccruedInterestUpdateJob(c *gin.Context) {
	go bondservice.UpdateAllBondsAci()
	c.JSON(http.StatusOK, shared.StringResponse{Message: "The bonds' accrued interest job has been successfully started"})
}
